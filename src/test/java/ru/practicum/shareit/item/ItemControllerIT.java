...
class ItemControllerIT {
    ...
    void getByItemId_whenInvoke_thenStatusOkItemBookedInBody() {
        Long itemId = 0L;
        ItemBooked itemBooked = ItemBooked.builder()
                .description("desc")
                .available(true).build();
        when(itemService.getByItemId(userId, itemId)).thenReturn(itemBooked);
    ...
    void getByItemId_whenItemNotFound_thenStatusNotFound() {
        Long itemId = 0L;
        when(itemService.getByItemId(userId, itemId))
                .thenThrow(EntityNotFoundException.class);
    ...
    void getAllItemsByUserId_whenInvoke_thenStatusOkItemBookedListInBody() {
        Integer from = 1;
        Integer size = 1;
        List<ItemBooked> itemBookedList = List.of(ItemBooked.builder()
                .description("desc")
                .available(true).build());
        when(itemService.getAllItemsDyUserId(userId, from, size)).thenReturn(itemBookedList);
